...
//        }
        ...
        if !modifiedIndexPaths.isEmpty {
                rootView.tableView.performBatchUpdates
                {
                    self.rootView.tableView.reloadRows(at: modifiedIndexPaths, with: .automatic)
                }
            }
//            
...
//        
//            if addedIndexPaths > 0 {
//                var indexPaths : [IndexPath] = []
//                for index in 0..<addedIndexPaths {
//                    let indexPath = IndexPath(row: index, section: 0)
//                    indexPaths.append(indexPath)
//                }
//                self.handleTableViewCellInsertion(with: indexPaths, scrollToBottom: false)
//            }
...
//        self.rootView.updateTableViewContentOffset(isInputBarHeaderRemoved: true)
    ...
    private func removeTableViewCells(at indexPath: [IndexPath])
    {
...
extension ChatRoomViewController {
    ...
    private func handleTableViewCellInsertion(with indexPaths: [IndexPath] = [IndexPath(row: 0, section: 0)],
                                              scrollToBottom: Bool)
    {
        isNewSectionAdded = checkIfNewSectionWasAdded()
        handleRowAndSectionInsertion(with: indexPaths, scrollToBottom: scrollToBottom)
//        rootView.tableView.endUpdates()
        if indexPaths.count == 1 {
            if let index = indexPaths.first {
                animateCellOffsetOnInsertion(usingCellIndexPath: index)
            }
        }
    ...
    private func handleRowAndSectionInsertion(with indexPaths: [IndexPath], scrollToBottom: Bool) {
        ...
        if !scrollToBottom && contentIsScrolled {
            UIView.animate(withDuration: 0.0) {
                self.rootView.tableView.insertRows(at: indexPaths, with: .none)
                self.rootView.tableView.reloadData()
            }
            return
        } else {
...
////                    }
//                    
//                    let row = changes.indices.map { IndexPath(row: $0, section: 0) }
////                    let row = addedIndexPaths.count == 0 ? 0 : addedIndexPaths.count + 1
////                    addedIndexPaths.append(IndexPath(row: row, section: 0))
//                case .removed(let removedIndex):
//                    removedIndexPaths.append(removedIndex)
////                    self.removeTableViewCells(at: [removedIndex])
//                case .modified(let indexPath, let modifiedValue):
...
//            self.handleTableViewCellsInsertion(with: addedIndexPaths, scrollToBottom: false)
//        }
//        
////        }
//    }
...
//    }
//    
//    private func handleTableViewCellsInsertion(with indexPaths: [IndexPath] = [IndexPath(row: 0, section: 0)],
//                                              scrollToBottom: Bool)
//    {
//        isNewSectionAdded = checkIfNewSectionWasAdded()
//        handleRowAndSectionInsertion(with: indexPaths, scrollToBottom: scrollToBottom)
////        rootView.tableView.endUpdates()
//        if indexPaths.count == 1, let index = indexPaths.first {
//            animateCellOffsetOnInsertion(usingCellIndexPath: index)
//        }
//        
//        isNewSectionAdded = false
