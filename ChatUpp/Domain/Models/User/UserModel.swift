...
actor MessagesSeenStatusUpdater
{
    func updateLocally(chatID: String,
                       authUserID: String,
                       isGroup: Bool,
...
actor UserUnseenMessageCounterUpdater
{
    func updateLocal(chatID: String,
                     authUserID: String,
...
//            RealmDatabase.shared.refresh()
            guard let conversation = RealmDatabase.shared.retrieveSingleObjectFromNewRalmInstance(
                ofType: Chat.self,
                primaryKey: chatID) else {
                return
            }
//            RealmDatabase.shared.update(object: conversation) { dbChat in
            RealmDatabase.shared.update() {
                if let participant = conversation.getParticipant(byID: authUserID)
                {
