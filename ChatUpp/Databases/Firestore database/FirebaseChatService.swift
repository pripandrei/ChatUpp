...
extension FirebaseChatService
{
    ...
    {
        let data: [String: Any] = [
            Message.CodingKeys.type.rawValue : type.rawValue,
        ]
        try await getMessageDocument(messagePath: messageID, fromChatDocumentPath: chatID).updateData(data)
    }
    ...
    ) async throws
    {
        ...
        while true
        {
            ...
            // Don't update pagination if we're stopping early
...
extension FirebaseChatService
{
                                   ...
                                   shouldIncrement: Bool) async throws
    {
        ...
        {
            let fieldPath = "participants.\(id).\(ChatParticipant.CodingKeys.unseenMessagesCount.rawValue)"
            // Subtract counter from current value atomically
            data[fieldPath] = FieldValue.increment(Int64(counter))
        }
...
//                guard messageDoc.data()["seen_by"] is [String] else {return}
    ...
    func migrateSeenByMapToArray() async throws
    {
        print("Starting migration of seen_by maps to arrays...")

        let chatsRef = db.collection("chats")

        // Only group chats (same logic you used before)
        ...
        for chatDoc in chatsSnapshot.documents {
            ...
            print("  Found \(messagesSnapshot.documents.count) messages")
            ...
            for messageDoc in messagesSnapshot.documents {
                ...
                // We only care about maps
                guard let seenByMap = data["seen_by"] as? [String: Bool] else {
                    continue
                }

                // Convert map → array (only true values)
                let seenByArray = seenByMap
                    .filter { $0.value == true }
                    .map { $0.key }

                // Optional: skip if already array-shaped (extra safety)
                if data["seen_by"] is [String] {
                    continue
                }
